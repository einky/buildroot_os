...
## No-motion mode ##############################################################
##
## transitions preference values:
##   2 = all transitions (default)
##   1 = only non-default transitions
##   0 = no transitions at all (kills hardcoded `with fade/dissolve` in scripts)
##
## This is the only setting that disables transitions from the script itself.
##
## Disable every form of animated motion: transitions, typewriter text,
## auto-forward, and skip animations.

# default_transition is a *store* variable (see renpy/defaultstore.py and
# the read in renpy/ast.py: `renpy.store.default_transition`). It's None by
# default — declared explicitly here so the intent is documented.
default preferences.transitions = 0

define default_transition = None

# Transition that runs between say-statements when character attributes change.
define config.say_attribute_transition = None

# Disable auto-forward mode at startup (afm_time above is also 0).
default preferences.afm_enable = False

# Disable skip-related animations and behavior.
